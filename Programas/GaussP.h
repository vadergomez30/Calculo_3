...
Mat invGaussJordan(const Mat& A){
    ...
    for(int col=0; col<n; col++){
        ...
        if(best<EPS) throw runtime_error("Matriz singular en inversion (pivote ~ 0).");
        if(piv!=col){ swap(M[piv],M[col]); swap(Inv[piv],Inv[col]); }
...
//procedimiento Gauss–Jordan particionado
// Pasos:
// 1) A11^{-1}. Luego A12' = A11^{-1} A12,  b1' = A11^{-1} b1
// 2) A22' = A22 - A21 A12',  b2' = b2 - A21 b1'
// 3) (A22')^{-1}. b2'' = (A22')^{-1} b2'
// 4) b1'' = b1' - A12' b2''
// 5) x = [b1''; b2'']
void GaussParticionado(){
    ...
    try{
        ...
        for(size_t i=0;i<b2.size();i++) b2_p[i] = b2[i] - A21b1p[i];

        printMat(A22_p, "A22' = A22 - A21 * A12'");
        printVec(b2_p,  "b2'  = b2 - A21 * b1'");

        // (Comprobacion de determinante por bloques, opcional)
        double detA11 = determinant(A11);
        double detA22p = determinant(A22_p);
        cout << "\nDet(A11): " << detA11 << "   Det(A22'): " << detA22p
        ...
        for(size_t i=0;i<b1_p.size();i++) b1_pp[i] = b1_p[i] - A12pb2pp[i];
