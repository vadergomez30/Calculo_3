...
// ================== Supuestos: dominancia diagonal + det != 0 ==================
Funcion ok <- Supuestos2(A, n)
...
// ================== Criterio de paro ==================
Funcion listo <- detener2(x_ant, x_nue, n, error)
...
// ================== Iteración Gauss-Seidel (recursiva) ==================
SubProceso IterativoGaussSeidel(A, n, b, x_ant, x_nue, error, count Por Referencia)
	Definir i, j Como Entero
	...
	Para i <- 1 Hasta n
		...
		Para j <- 1 Hasta n
			Si j <> i Entonces
				// Uso de x_nue (valores ya actualizados) como en el C++
				res <- res - A[i,j] * x_nue[j]
			FinSi
		FinPara
		x_nue[i] <- res / A[i,i]
	FinPara
	...
	Si detener2(x_ant, x_nue, n, error) Entonces
	...
	SiNo
		...
		Para i <- 1 Hasta n
			x_ant[i] <- x_nue[i]
		FinPara
		IterativoGaussSeidel(A, n, b, x_ant, x_nue, error, count)
	FinSi
FinSubProceso

// ================== Principal: GaussSeidel() ==================
