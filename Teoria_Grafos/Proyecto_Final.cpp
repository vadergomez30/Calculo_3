...
int main() {
    ...
    while(repetir == 's' || repetir == 'S'){
        ...
        tipoGrafica = stoi(inputTipo);
        {
            ...
            for(int i=0; i<ver; i++){
                for(int j=0; j<lin; j++){
                    ...
                    if(tipoGrafica==2){
                        if(incidencia[i][j]==2){ grados2[i][0]+=1; grados2[i][1]+=1; }
                        else{
            ...
            if(tipoGrafica==1){ 
                cout<<"Grado: \n";
                for(int i=0; i<ver; i++) cout<<v[i]<<": "<<grados1[i]<<'\n';
                cout<<'\n';
                ...
                for(int i=0; i<ver; i++){
                    cout<<v[i]<<": ";
                    for(int j=0; j<2; j++) cout<<grados2[i][j]<<"    ";
                    cout<<'\n';
                }
                cout<<"Caracteristicas: \n";
                for(int i=0; i<ver; i++){
            ...
            for(int j = 0; j < lin; j++){
                ...
                for(int i = 0; i < ver; i++) columna[i] = incidencia[i][j];        
            ...
            for(const auto& [col, indices] : paralelas){
                if(indices.size() > 1){
                    simple=false;
                    cout<<"Las lineas ";
                    for(int ind : indices) cout<<ind+1<<" ";
                    cout<<"son paralelas\n";
                }
            }    
            if(tipoGrafica==2){
                for(int i=0; i<ver; i++)
                    for(int j=0; j<lin; j++)
                        if(incidencia[i][j]==2){ simple=false; cout<<"La linea "<<j+1<<" es un bucle\n"; }
            } else {
                for(int j=0; j<lin; j++){
                    int sum=0;
                    for(int i=0; i<ver; i++) sum+=incidencia[i][j];
                    if(sum==1){ simple=false; cout<<"La linea "<<j+1<<" es un bucle\n"; }
                }
            }
            ...
            for(int i=0; i<ver; i++){
                if(grados1[i]==2){
                    for(int j=0; j<lin; j++){
                        if(incidencia[i][j]==1){
                            vector<ll> colum(ver);
                            for(int h=0; h<ver; h++) colum[h]=incidencia[h][j];
            ...
            for(int i=0; i<ver; i++)
                for(int j=0; j<ver; j++)
                    if(accesibilidad[i][j]==0){ conectada=false; goto salir; }
            salir:
            ...
            if(tipoGrafica==1){
                for(int i=1; i<ver; i++)
                    if(grados1[i]!=grados1[i-1]){ regular=false; goto salir1; }
            } else {
                ll grad=grados2[0][0];
                for(int i=0; i<ver; i++){
                    if(grados2[i][0]!=grados2[i][1]){ balanceada=false; regular=false; goto salir1; }
                    if(grad!=grados2[i][0] || grad!=grados2[i][1]) regular=false;
                }
            }
            salir1:
            if(regular) cout<<"La Grafica es regular\n";

            if(tipoGrafica==1){ if(ver>1 && simple && lin==(ver*(ver-1)/2)) cout<<"La Grafica es completa\n"; }
            else { if(ver>1 && simple && lin==(ver*(ver-1))) cout<<"La Grafica es completa\n"; }
            ...
            for(int i=0; i<ver; i++)
                for(int j=0; j<ver; j++)
                    if(adyacencia[i][j]!=adyacencia[j][i]){ simetrica=false; goto salir2; }
            salir2: 
            ...
            if(tipoGrafica==1 && conectada){
                int gradpar=0;
                for(int i=0; i<ver; i++) if(grados1[i]%2==0) gradpar++;
            ...
            if(tipoGrafica==2 && !balanceada){
                int gradigual=0;
                for(int i=0; i<ver; i++) if(grados2[i][0]==grados2[i][1]) gradigual++;
        ...
        preguntar:
        cout<<"\n¿Deseas ingresar otra grafica? (s/n): ";
        cin>>repetir;
        cout<<'\n';
    }
